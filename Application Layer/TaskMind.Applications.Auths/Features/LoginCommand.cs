...
{
    /// <summary>Đăng nhập dùng chung cho mọi loại tài khoản (User/Staff/Teacher/Student/AdminCompany/AdminSchool/Admin) — mục 4.1.</summary>
    ...
    public class LoginCommandHandler : IRequestHandler<LoginCommand, LoginResultDto>
    {
        ...
        public async Task<LoginResultDto> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Password))
                throw new InvalidOperationException("Vui lòng nhập đầy đủ email và mật khẩu.");
            ...
            if (account.Status == EntityStatus.Paused)
                throw new UnauthorizedAccessException("Tài khoản đang bị tạm khoá.");
            ...
            if (!tokenResult.IsSuccess)
                throw new InvalidOperationException(tokenResult.Message);
